...
enum class Stat {
    ...
    /** Clears with no sniff and no treat spent. */
    BoosterFreeClears,
 ...
 * numbers". Counters come in four shapes and only one of them is a sum, which
 ...
 */
data class AchievementCounters(private val values: Map<Stat, Long> = emptyMap()) {
    ...
    fun fold(result: LevelResult): AchievementCounters {
        ...
        // opposite of flawless, so a failure has to break the streak.
        val flawless = result.completed && result.strikes == 0
        val streak = if (flawless) this[Stat.FlawlessStreak] + 1L else 0L
        next[Stat.FlawlessStreak] = streak
        setMax(Stat.BestFlawlessStreak, streak)
        ...
        // streaks, and a later release can ask it new questions — but it earns
        // nothing.
        if (!result.completed) return AchievementCounters(next)
        ...
        if (flawless) inc(Stat.FlawlessClears)
        if (result.strikes >= COMEBACK_STRIKES) inc(Stat.ComebackClears)
        if (result.sniffsUsed == 0 && result.treatsUsed == 0) inc(Stat.BoosterFreeClears)
        if (result.paws >= THREE_PAWS && result.previousBestPaws < THREE_PAWS) inc(Stat.ThreePawClears)
        ...
        if (result.size >= BIG_BOARD_SIZE && result.timeMs in 1..BIG_BOARD_SPRINT_MS) {
            inc(Stat.BigBoardSprintClears)
        }
        ...
        return AchievementCounters(next)
    }
