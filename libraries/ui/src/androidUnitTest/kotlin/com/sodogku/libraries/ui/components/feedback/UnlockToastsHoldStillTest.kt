...
/**
 * A badge toast slides in and slides away when its time is up, and does both
 * in place instead when the player has asked for fewer animations, without
 * losing the timer that takes it away. Under inspection it does nothing at all.
 *
 ...
 * ### The entrance
 *
 * The first version's `AnimatedVisibility` started visible, so its enter
 * transition never ran and the only slide that reached the screen was the one
 * out. The rebuild starts the toast hidden and shows it in the same
 * composition, and [aToastSlidesInWhenNothingIsWatching] is the guard against
 * going back: a toast that starts visible writes nothing during its first half
 * second, and this counts those writes.
 *
 * ### What is measured
 *
 * Snapshot writes while a transition would be running, the measure every
...
class UnlockToastsHoldStillTest {
    ...
    @Test
    fun aToastSlidesInWhenNothingIsWatching() {
        val changes = changesWhileTheToastArrives(reduceAnimations = false, inspecting = false)

        assertTrue(
            changes > MinMovingChanges,
            "a toast produced $changes state writes during its entrance, so it is starting " +
                "visible again and the slide in never runs",
        )
    }

    @Test
    fun aToastAppearsInPlaceWhenThePlayerAskedForFewerAnimations() {
        assertEquals(
            0,
            changesWhileTheToastArrives(reduceAnimations = true, inspecting = false),
            "the reduce-animations setting no longer reaches the badge toast's entrance",
        )
    }
    ...
    /**
     * Under inspection nothing moves and nothing is scheduled: a preview or a
     * screenshot waits for idle, and a toast still counting down its dwell
     * is not idle. So it is there, it stays there, and it writes nothing.
     */
    @Test
    fun aToastHoldsStillUnderInspection() {
        var dismissed = 0
        compose.mainClock.autoAdvance = false
        compose.setContent {
        ...
        assertEquals(0, changes, "a toast under inspection kept writing state, which a screenshot test waits on forever")
        assertEquals(0, dismissed, "a toast under inspection ran its timer")
        compose.onNodeWithText(Item.title).assertExists()
    }

    private fun changesWhileTheToastArrives(reduceAnimations: Boolean, inspecting: Boolean): Int {
        compose.mainClock.autoAdvance = false
        var changes = 0
        val handle = Snapshot.registerApplyObserver { changed, _ -> changes += changed.size }
        return try {
            ...
            // The first frame is composition itself, which writes whatever it
            // writes with or without a transition. The count starts after it.
            compose.mainClock.advanceTimeByFrame()
            changes = 0
            compose.mainClock.advanceTimeBy(EnterWindowMillis)
            compose.waitForIdle()
            changes
        } finally {
            handle.dispose()
    ...
    private companion object {
        ...
        /** Long enough for the slide in, however it is tuned. */
        const val EnterWindowMillis = 600L

        /** The toast's own dwell plus a frame or two. */
        const val JustPastTheDwellMillis = 4_100L
        ...
        const val PastTheDwellMillis = 5_500L
