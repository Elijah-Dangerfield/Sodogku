...
| S8 | A trophy icon with a badge count beside the settings gear on the puzzle screen, the badge persisting until the player opens it | **DONE** (2026-09-08, `6f339ae`) — badge counts unlocks since `AppData.achievementsSeenAt`, cleared by opening the grid from anywhere |
...
| S13 | Game Center on iOS | **DONE** (2026-09-08, `bdac05e` + `647d4a8` + `6f339ae`) — module, app graph, Settings entry point, and both boards posted on every clear. The two leaderboard records are owner work (`OWNER-TODO.md` item 4) |
...
| S16 | "Beat 84.6% of players" on the win dialog. Decide whether that is real data or a qualitative message derived from the player's own run | **DONE** (2026-09-08, `9a39641` + `6f339ae` + `b4d1ca7`) — qualitative, judged against par, rendered in the title slot so the sheet gains no line. `b4d1ca7` fixed the top verdict being unreachable |
